...
def test_logger_creation(logger_instance, request):
    ...
    ), f"Expected '{expected_logger_name}', got '{logger.name}'"
...
def test_console_logging_enabled(caplog):
    ...
    assert (
        "Console test message" in caplog.text
...
def test_console_logging_disabled(caplog):
    ...
    assert (
        "This should not be logged" not in caplog.text
    ), "Console logging was incorrectly enabled"
...
def test_file_logging_disabled(temp_log_dir, request):
    ...
    log_name = request.node.name
    logger = DarcaLogger(
        name=log_name, log_to_file=False, log_directory=temp_log_dir
    ).get_logger()
    ...
    assert not os.path.exists(
        log_file_path
    ), "Log file was created despite being disabled"
...
def test_logger_singleton():
    ...
    assert (
        logger1 is logger2
    ), "Logger instances are not the same (singleton failed)"
