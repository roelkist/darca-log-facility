...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-log-venv
    export POETRY_HOME := /tmp/poetry-log-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-log-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/log-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-log-cache
endif
